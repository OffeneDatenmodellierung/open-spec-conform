...
//! trip at a time.
//!
//! The pre-existing `validate_odcs_internal` is worse than that bound
//! suggests, because the `jsonschema` call underneath it short-circuits:
//!
//! ```text
//! if let Err(error) = validator.validate(&data) {
//!     return Err(format_validation_error(&error, "ODCS"));
//! }
//! ```
//!
...
//!
//! ```text
//! ODCS validation failed at path '/apiVersion': "v9.9.9" is not one of …
//! ```
//!
//! and nothing else.
...
fn a_document_with_many_faults_reports_many_errors() {
    ...
    assert!(
        errors.len() >= 7,
        "expected at least seven separate errors from a document built to carry seven \
...
fn those_errors_are_of_several_different_kinds() {
    ...
    // one thing.
    for expected in [
        conform_odcs::codes::REQUIRED_MISSING,
        conform_odcs::codes::UNKNOWN_PROPERTY,
        conform_odcs::codes::VALUE_NOT_PERMITTED,
        conform_odcs::codes::WRONG_TYPE,
    ] {
        assert!(
...
fn every_error_says_where_it_is() {
    ...
    for diagnostic in report.at_or_above(Severity::Error) {
        ...
        let root_level = matches!(
            ...
            conform_odcs::codes::REQUIRED_MISSING | conform_odcs::codes::UNKNOWN_PROPERTY
        );
        assert!(
...
fn every_error_carries_the_standard_it_was_raised_under() {
    ...
    for diagnostic in report.at_or_above(Severity::Error) {
        ...
        assert_eq!(spec.id, conform_odcs::SPEC_ID);
        assert_eq!(
