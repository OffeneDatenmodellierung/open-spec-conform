...
use conform_odcs::{OdcsValidator, codes};
...
impl Scratch {
    ...
    fn with_schema(&self, edit: impl FnOnce(String) -> String) -> &Self {
        let source = support::workspace_root().join("schemas/odcs-json-schema-v3.1.0.json");
        let text = fs::read_to_string(&source)
            .unwrap_or_else(|e| panic!("cannot read {}: {e}", source.display()));
        fs::write(
            self.root.join("schemas/odcs-json-schema-v3.1.0.json"),
            edit(text),
        )
...
fn real_digest() -> String {
    let bytes = fs::read(support::workspace_root().join("schemas/odcs-json-schema-v3.1.0.json"))
        .expect("cannot read the vendored schema");
    conform_registry::sha256_hex(&bytes)
}

fn codes_in(error: &conform_odcs::SchemaError) -> Vec<String> {
    error
        .report()
        .iter()
...
fn an_honest_registry_builds_a_validator() {
    ...
    let validator = OdcsValidator::from_registry_path(&registry)
...
fn a_schema_that_no_longer_matches_its_digest_is_refused() {
    ...
    let error = OdcsValidator::from_registry_path(&registry)
