...
use crate::rules::{self, SchemaFacts};
use crate::schema;

/// The identifier this crate looks for in `specs.toml`.
///
/// `odcl`, not `lexicon`: the registry is the single source of truth for this
/// standard's provenance and `odcl` is the id it issues, so that is what this
/// crate resolves and what its diagnostic codes are namespaced to. A registry
/// that renames the entry is a registry this crate cannot find its schema in,
/// and it will say so under [`codes::NOT_IN_REGISTRY`] rather than fall back
/// to a guess.
pub const SPEC_ID: &str = "odcl";

/// Validates Open Data Contract Lexicon documents against the schema the
/// registry pins.
///
...
///
/// 1. finds the `odcl` entry in the registry — a schema this repository has
...
/// 3. compiles what it read, and reads out of it the facts the rules in
///    [`crate::rules`] need, so none of them transcribes a list the schema
///    already holds.
///
...
/// published standard is not a conformance verdict at all. The pre-existing
/// `validate_odcl_internal` `include_str!`s its copy with no digest, no
/// version in the path and no record of where it came from; this is the check
/// that stops that happening again here.
///
/// # What it produces
///
...
///   this crate's pass/fail verdict is the schema's verdict — and therefore
///   the pre-existing validator's;
/// - **warnings** are the hygiene rules and the cross-references that were
///   followed and found absent;
/// - **info** notes say which schema was used and that its bytes were
///   verified, and record every reference that resolved or was deliberately
///   not followed.
...
impl Validator for LexiconValidator {
    ...
    fn validate(&self, document: &Document) -> ConformanceReport {
        ...
        report.extend(rules::beyond_the_schema(
            &instance,
            &self.facts,
            id,
            &self.spec,
...
// came from.
impl fmt::Debug for LexiconValidator {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("LexiconValidator")
