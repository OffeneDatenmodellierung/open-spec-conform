...
//!
//! `data-modelling-sdk`'s `validate_odcl_internal` returns `Result<(), String>`.
...
//! failures here.
//!
//! # Why this oracle is a clean one
//!
//! Worth stating, because its ODCS sibling is not. `validate_odcs_internal`
//! sniffs its input and hands ODCL-shaped documents to `validate_odcl_internal`
//! while returning the result as though ODCS had been checked — recorded as
//! F-001 in `docs/findings/0001-upstream-defects.md`, and the reason
//! `conform-odcs`'s differential test carries a known divergence.
//!
//! `validate_odcl_internal` does none of that. It compiles the ODCL schema and
//! validates against it, full stop. So there is no dispatch to see through
//! here, and any disagreement below would be a disagreement about rules.
//!
...
//! schema-validation`, and calls `validate_odcl_internal` directly over every
//! file in `tests/fixtures/`. What it returned is committed verbatim in
//! `tests/oracle/odcl-verdicts.json`. Nothing here is a reimplementation of
...
//!
//! # Disagreements would be recorded, not smoothed over
//!
//! [`KNOWN_DIVERGENCES`] lists every fixture where the two verdicts differ,
//! each with the reason. It is **empty**, and that is a result rather than an
//! omission: over eighteen fixtures splitting nine/nine, this crate and the
//! function it replaces agree everywhere.
//!
//! It is not a suppression list, and the difference is enforced in both
//! directions: an unlisted fixture that diverges fails this test, and a listed
//! fixture that stops diverging fails it too. So the list cannot quietly
//! become an exemption for a regression.
...
/// and why.
///
/// Empty, and deliberately kept as a populated-shaped constant rather than
/// deleted: the machinery that would report a divergence has to exist before
/// one appears, or the first real one gets resolved by whoever finds it
/// instead of recorded.
...
fn recording() -> Value {
    let path = support::fixtures_dir()
        ...
        .join("oracle/odcl-verdicts.json");
    let text =
...
#[test]
fn the_recording_is_of_the_function_this_crate_replaces() {
    let recording = recording();
    assert_eq!(
        recording["oracle"]["function"],
        Value::String("data_modelling_core::validation::schema::validate_odcl_internal".to_owned()),
        "the recording is of a different function, so it says nothing about this crate"
    );
    assert!(
        recording["oracle"]["source_commit"].is_string(),
        "the recording does not say which revision of the SDK it was taken from, so \"which \
         implementation was this recorded against\" has no answer"
    );
}
...
/// Same verdict is half the claim. This is the other half.
///
/// The oracle returns at most one `String` per document. If this crate also
/// returned one finding per document it would agree on every verdict above and
/// be worth nothing, so the corpus has to prove that the detail is really
/// there.
#[test]
fn the_detail_the_oracle_threw_away_is_actually_recovered() {
    let validator = support::validator();

    let report = validator.validate(&support::fixture("faulty-many-faults.yaml"));
    let errors = support::errors(&report);
    assert!(
        errors.len() >= 4,
        "the whole argument for this crate is that one document yields many findings, and this \
         one yielded {}: {errors:?}",
        errors.len()
    );

    // Every error carries the three things a `String` cannot.
    for diagnostic in report.at_or_above(conform_core::Severity::Error) {
        assert!(
            !diagnostic.code.as_str().is_empty(),
            "a finding with no stable code cannot be suppressed or counted: {diagnostic}"
        );
        assert!(
            diagnostic.spec_ref.is_some(),
            "a finding that cannot say which clause objected is not traceable: {diagnostic}"
        );
    }
    assert!(
        report
            .at_or_above(conform_core::Severity::Error)
            .any(|d| d.location.pointer.is_some()),
        "no finding carried a JSON Pointer, so nothing here could underline anything"
    );
}
