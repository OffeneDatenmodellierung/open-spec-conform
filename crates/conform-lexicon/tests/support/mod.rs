...
pub fn workspace_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .expect("crates/conform-lexicon should sit two levels below the workspace root")
        .to_path_buf()
}
...
pub fn validator() -> LexiconValidator {
    let path = workspace_root().join("specs.toml");
    LexiconValidator::from_registry_path(&path).unwrap_or_else(|error| {
        panic!(
...
/// empty". A conformant document still collects hygiene warnings, reference
/// notes and a provenance note, and a test that asked for silence would be
/// asserting the wrong thing.
pub fn fails(validator: &LexiconValidator, document: &Document) -> bool {
    validator
        .validate(document)
...
pub fn count_code(report: &ConformanceReport, code: &str) -> usize {
    report.iter().filter(|d| d.code.as_str() == code).count()
}
