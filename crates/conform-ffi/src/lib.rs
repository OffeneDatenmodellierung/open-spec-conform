...
//! # The header
//!
//! `include/conform.h` is generated by `cbindgen` and committed.
//! `tests/the_header_matches_the_library.rs` regenerates it and fails if the
//! committed copy differs, so it cannot rot. That test needs `cbindgen` and is
//! therefore gated behind `--features header-check`; the C smoke test needs a
//! C compiler and is gated behind `--features c-smoke`. A plain `cargo test
//! --workspace` builds neither, which is the point: nobody linking this
//! library should have to compile a header generator.
//!
//! # Example, from Rust
//!
...
///
/// The other two numbers a consumer may want are
/// [`abi::conform_version`], which is this crate's release version, and
/// [`SCHEMA_VERSION`], which is the shape of the JSON that comes back.
