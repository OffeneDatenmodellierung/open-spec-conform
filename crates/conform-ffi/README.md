...
committed copy differs, so it cannot rot.

## Building and testing

A plain `cargo test --workspace` builds this crate's library and its three
default-feature test files, and needs no C toolchain and no header generator.
The two tests that need more are gated:

```sh
cargo test -p conform-ffi --features header-check   # needs nothing extra; builds cbindgen
cargo test -p conform-ffi --features c-smoke        # needs a C compiler on PATH as `cc`
cargo test -p conform-ffi --all-features            # both
```

`cbindgen` is an *optional* dependency rather than a dev-dependency because
cargo builds every dev-dependency of every workspace member whenever a test
target is built, and nobody linking this library should have to compile a
`syn` tree to do it.
