...
pub const SPEC_ID: &str = "odps";

/// Validates Open Data Product Standard documents against the schema the
/// registry pins.
///
...
///
/// 1. finds the `odps` entry in the registry — a schema this repository has
...
/// published standard is not a conformance verdict at all. This crate's schema
/// is the one that makes the point: it was vendored in `data-modelling-sdk` as
/// `odps-json-schema-latest.json`, with no version, no source URL, no fetch
/// date and no `$id`, so nothing could answer "is this current?". This is the
/// check that stops that happening quietly a second time.
///
/// # What it produces
///
...
impl OdpsValidator {
    ...
    /// if the registry has no `odps` entry, if the vendored schema is missing,
...
impl Validator for OdpsValidator {
...
// came from.
impl fmt::Debug for OdpsValidator {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("OdpsValidator")
