...
/// `$schema` keyword. The vendored ODPS schema declares 2019-09; hard-coding
/// that here would mean a future schema that moved draft was validated under
/// the wrong rules while still looking green.
pub(crate) fn compile(
    schema: &Value,
) -> Result<jsonschema::Validator, jsonschema::ValidationError<'static>> {
    // `options()` rather than `validator_for()` so `format` keywords are
    // actually checked. From draft 2019-09 onward `format` is an annotation by
    // default, which would silently reduce `format: uri` to documentation.
    jsonschema::options()
...
fn code_for(kind: &ValidationErrorKind) -> &'static str {
    match kind {
        ...
        | ValidationErrorKind::PropertyNames { .. } => codes::UNKNOWN_PROPERTY,
    ...
    match serde_norway::from_str::<Value>(text) {
        Ok(Value::Null) => Err(Box::new(
            Diagnostic::error(
                ...
                "document is empty: there is no data product here to check",
            )
            .with_help("a data product needs at least `apiVersion`, `kind`, `id` and `status`")
