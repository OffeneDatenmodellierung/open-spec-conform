//! Diagnostic-rich conformance validation for **Open Data Product Standard**
//! documents.
//!
//! # Why this exists
//!
//! ODPS documents in this estate were already being validated. The function
...
//! product" arrive identically. It carries no stable code, so a CI job that
...
//! beside it returns `Ok(())` without looking at the document at all. With the
...
//! pre-existing `validate_odps_internal`'s over a fixture corpus and fails on
//! any disagreement that is not recorded, explained and proven to still
//! diverge.
//!
//! # Where the schema comes from
//!
//! Not from a hard-coded path, and this crate is the reason that rule exists.
//! The ODPS schema in `data-modelling-sdk` is vendored as
//! `odps-json-schema-latest.json`: no version, no source URL, no fetch date,
//! no `$id`. Nothing about those bytes could answer "is this still what Bitol
//! publishes?".
//!
//! [`OdpsValidator::from_registry`] resolves the schema through
//! [`conform_registry`] instead, which means the bytes are re-hashed against
//! the digest `specs.toml` records for them *before* anything is validated
//! against them, and the entry they came from names an immutable upstream pin.
//! A schema that has drifted is refused, loudly.
//!
//! # Example
//!
//! ```no_run
...
//! ));
//!
//! // `id` and `status` are both absent, and each is its own finding — the
//! // thing `Result<(), String>` has no way to express.
//! for diagnostic in report.at_or_above(Severity::Error) {
//!     println!("{diagnostic}");
//! }
//! assert!(report.should_gate(GatePolicy::default()));
//! # Ok::<(), conform_odps::SchemaError>(())
//! ```
//!
...
pub use validator::{OdpsValidator, SPEC_ID, SchemaError};
