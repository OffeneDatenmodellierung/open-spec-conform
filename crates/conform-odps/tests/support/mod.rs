...
pub fn workspace_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .expect("crates/conform-odps should sit two levels below the workspace root")
        .to_path_buf()
}
...
pub fn validator() -> OdpsValidator {
    let path = workspace_root().join("specs.toml");
    OdpsValidator::from_registry_path(&path).unwrap_or_else(|error| {
        panic!(
...
/// wrong thing.
pub fn fails(validator: &OdpsValidator, document: &Document) -> bool {
    validator
        .validate(document)
