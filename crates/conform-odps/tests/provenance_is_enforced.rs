...
use conform_odps::{OdpsValidator, codes};
...
impl Scratch {
    ...
    fn with_schema(&self, edit: impl FnOnce(String) -> String) -> &Self {
        let source = support::workspace_root().join("schemas/odps-json-schema-v1.0.0.json");
        let text = fs::read_to_string(&source)
            .unwrap_or_else(|e| panic!("cannot read {}: {e}", source.display()));
        fs::write(
            self.root.join("schemas/odps-json-schema-v1.0.0.json"),
            edit(text),
        )
...
[[spec]]
id            = "odps"
name          = "Open Data Product Standard"
version       = "1.0.0"
homepage      = "https://bitol-io.github.io/open-data-product-standard"
repository    = "https://github.com/bitol-io/open-data-product-standard"
steward       = "Bitol"
licence       = "Apache-2.0"
pinned_ref    = "v1.0.0"
...
fn real_digest() -> String {
    let bytes = fs::read(support::workspace_root().join("schemas/odps-json-schema-v1.0.0.json"))
        .expect("cannot read the vendored schema");
    conform_registry::sha256_hex(&bytes)
}

fn codes_in(error: &conform_odps::SchemaError) -> Vec<String> {
    error
        .report()
        .iter()
...
fn an_honest_registry_builds_a_validator() {
    ...
    let validator = OdpsValidator::from_registry_path(&registry)
...
fn a_schema_that_no_longer_matches_its_digest_is_refused() {
    ...
    let error = OdpsValidator::from_registry_path(&registry)
