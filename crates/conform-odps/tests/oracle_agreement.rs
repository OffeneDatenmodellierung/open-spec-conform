...
//!
//! `data-modelling-sdk`'s `validate_odps_internal` returns `Result<(), String>`.
...
//! schema-validation`, and calls `validate_odps_internal` directly over every
//! file in `tests/fixtures/`. What it returned is committed verbatim in
//! `tests/oracle/odps-verdicts.json`. Nothing here is a reimplementation of
...
//!
//! **It is currently empty.** Across the whole ODPS corpus the two validators
//! agree on every document. That is the result this crate wanted and it is
//! worth stating plainly rather than leaving as an absence: the ODPS oracle
//! does no format sniffing, so there is nothing here of the kind its ODCS
//! sibling turned up.
...
/// and why.
///
/// Empty, and that is the finding: over the whole corpus the two agree. An
/// entry here would need a written reason, and a listed fixture that stopped
/// diverging would fail the test below — so this staying empty is checked,
/// not assumed.
...
fn recording() -> Value {
    let path = support::fixtures_dir()
        ...
        .join("oracle/odps-verdicts.json");
    let text =
